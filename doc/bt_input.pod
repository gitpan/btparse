...
in the value from all comment entries; the AST returned by one of the
C<bt_parse_*> functions will reflect this change.
...
assuming that C<get_more_text()> does as its name implies, and returns
C<NULL> when there's no more text available.
